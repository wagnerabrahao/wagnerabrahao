...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        /* CORREÇÃO DEFINITIVA: Títulos, Resumos e Assinaturas em BRANCO */
        .livro-titulo { color: var(--white) !important; font-family: 'Bodoni Moda'; margin-bottom: 15px; text-transform: uppercase; }
        .livro-resumo { color: var(--white) !important; opacity: 0.9; font-style: italic; margin-top: 15px; line-height: 1.7; }
        .livro-assinatura { color: var(--white) !important; margin-top:20px; font-family:'Great Vibes'; font-size:1.8rem; }
...
não anestesiado pela falta de sensação, mas pelo excesso controlado. 
Excesso de rotina, de precisão, de identidades fixas. 
Sua existência era um Lá central perpétuo: afinado, correto, mas sem melodia.
Até que perdeu o filho.
E a perda quebrou algo fundamental: o mecanismo de controle que mantinha mundo arrumado, sentimentos guardados, corpo obediente.
O que veio depois não foi colapso. Foi revelação.</div>
            </div>
            ...
            <div class="verbo-text-block">
                <div class="verbo-header">A Fogueira Espiritual</div>
                <div class="verbo-content">"A luz que você busca no silêncio do outro é o reflexo da centelha que já aquece o seu próprio coração. No invisível, somos todos chamas de uma mesma fogueira espiritual."</div>
            </div>
        ...
        <div id="obras" class="page">
            ...
            <div class="livros-grid">
                ...
                <div class="caixa-dourada"><h3 class="livro-titulo">A Ressonância das Almas</h3><p class="livro-resumo">Saga alquímica sobre a conexão das frequências espirituais através das gerações.</p><div class="livro-assinatura">Wagner Abrahão</div></div>
            </div>
        </div>
    ...
    <script>
        ...
        const silvM = new THREE.MeshStandardMaterial({ color: 0xEEEEEE, metalness: 1, roughness: 0.1 });
        
        let tiers = 100; let base = 150;
        for(let i=0; i<tiers; i++) {
            let curS = base * (1 - i/tiers); if(curS < 1.5) break;
            let geo = new THREE.BoxGeometry(curS/2, 2.5, curS);
            let bL = new THREE.Mesh(geo, stoneM); bL.position.set(-curS/4, i*2.5, 0); pyramid.add(bL);
            let bR = new THREE.Mesh(geo, stoneM); bR.position.set(curS/4, i*2.5, 0); pyramid.add(bR);
            if(i % 10 === 0) {
                let eL = new THREE.Mesh(new THREE.BoxGeometry(curS/10, 2.6, 2.6), goldM); eL.position.set(-curS/4, i*2.5, 0); pyramid.add(eL);
                let eR = new THREE.Mesh(new THREE.BoxGeometry(curS/10, 2.6, 2.6), silvM); eR.position.set(curS/4, i*2.5, 0); pyramid.add(eR);
            }
        }
        const cap = new THREE.Mesh(new THREE.ConeGeometry(8, 12, 4), goldM); cap.position.y = tiers*2.5 + 4; pyramid.add(cap);
        ...
        owl.add(eL, eR); owl.position.y = cap.position.y + 6; pyramid.add(owl);
        pyramid.position.y = -100; scene.add(pyramid);
